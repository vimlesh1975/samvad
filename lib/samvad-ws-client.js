...
const clientVersion = 'right-align-v40';
...
class SamvadWsClient {
  ...
  async sendRunorderDirection(rtl) {
    ...
    return {
      ...
      rightAlignColumns: getRightAlignColumns(),
      count: sent.length,
      sent,
    };
  }
...
function applyTextDirectionToSmvd(value, rtl) {
  ...
  return String(value ?? '')
    .split('\n')
    .map((line) => applyTextDirectionToSmvdLine(line, rtl, rightAlignColumns))
    .join('\n');
}

function applyTextDirectionToSmvdLine(line, rtl, rightAlignColumns) {
  if (!line.trim()) {
    return line;
  }
  ...
  const rightAlignMarker = '\u2060\u2063\u2060';
  const cleanText = text
    .replace(new RegExp(`${rightAlignMarker}[ \\t]*`, 'g'), '')
    .replace(/[\u200e\u200f\u202a-\u202e\u2066-\u2069]/g, '');
  ...
  if (!rtl) {
    return `${leadingTokens}${cleanText}`;
  }

  const textToAlign = cleanText.trimStart();
  const visibleLength = Array.from(textToAlign.trimEnd()).length;
  const padding = ' '.repeat(Math.max(0, rightAlignColumns - visibleLength));
  return `${leadingTokens}${rightAlignMarker}${padding}${textToAlign}`;
}

function getRightAlignColumns() {
  const columns = Number(process.env.SAMVAD_RIGHT_ALIGN_COLUMNS);

  if (Number.isFinite(columns)) {
    return Math.min(160, Math.max(20, Math.round(columns)));
  }
